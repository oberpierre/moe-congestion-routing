...
Fetches the datasets for the seven benchmark tasks the evaluation suite runs (openbookqa,
winogrande, piqa, arc_easy, arc_challenge, hellaswag, wikitext) plus the `EleutherAI/pythia-12b`
...
Dataset identifiers (HF `dataset_path`/`dataset_name`) are read from the pinned
...
# This list is fixed deliberately rather than derived. The first six are the suite FLAME-MoE
# reports which is what makes our numbers readable next to theirs. wikitext serves as a
# tokenizer-invariant perplexity comparison. Adding a task means re-running this script.
TASKS = [
    "openbookqa",
    "winogrande",
    "piqa",
...
def _task_dataset_ids() -> dict[str, tuple[str, str | None]]:
    ...
    for name in TASKS:
        ...
        if entry is None:
            raise KeyError(
                ...
                "the pinned harness and this script's task list have drifted"
            )
